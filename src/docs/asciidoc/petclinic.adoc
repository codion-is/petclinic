...
==== UI
We extend the {doc-url}/api/is.codion.swing.framework.ui/is/codion/swing/framework/ui/EntityEditPanel.html[EntityEditPanel] class, with a constructor taking a `SwingEntityEditModel` parameter, which we simply propagate to the super constructor. We implement the `initializeUI()` method in which we create the components using the `create...()` methods and add them to the panel using `addInputPanel()`.
